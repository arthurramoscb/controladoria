...
#df_trab_ger = compress_values(df_trab_ger, ["TERCEIRO_PRINCIPAL","PROCESSO_ESTADO"])

df_trab_ger.createOrReplaceTempView("df_gerencial_compara")
...
df_fechamento_trab_2.createOrReplaceTempView(f'df_fechamento_trab_2')

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC SELECT A.*, B.PROCESSO_ESTADO FROM df_fechamento_trab_2 A 
# MAGIC LEFT JOIN df_gerencial_compara B
# MAGIC ON A.`ID PROCESSO` = B.PROCESSO_ID
# MAGIC WHERE A.`ID PROCESSO` IN (1145312,1146378,1143700)
...
# Copy the file from the local disk to the desired volume
volume_path = f'/Volumes/databox/juridico_comum/arquivos/modelo_provisao/output/FECH_TRAB_MODELAGEM_{nmtabela_trab_ger}_F.xlsx'
