...
#df_ff_trab = spark.sql("select * from databox.juridico_comum.geral_fechamento_trabdatabox.juridico_comum.geral_fechamento_trab")
...
df_ff_trab = df_ff_trab.withColumn(
    ...
    # .when(col("ESTRATEGIA").isNull(), "SEM ESTRATEGIA")
...
# Ajusta tipo data
#lista_data = ['CADASTRO','DATACADASTRO']
...
df_ff_trab_consolidado = df_ff_trab_consolidado.withColumn(
    ...
    when(col("INDICACAO_PROCESSO_ESTRATEGICO").contains("AC"), "ACORDO")
    .when(col("INDICACAO_PROCESSO_ESTRATEGICO").contains("DE"), "DEFESA")
    .otherwise("SEM ESTRATEGIA")
)
...
# display(df_ger_consolidado.limit(5))

# COMMAND ----------

df_ger_consolidado = df_ger_consolidado.withColumnRenamed(
    'NOVO_TERCEIRO_CNPJ_RAZAO_SOCIAL_RAZAO_SOCIAL', 
    'NOVO_TERCEIRO',
    #'TERCEIRO_NOVO'
)
...
df_ger_consolidado.display()

# COMMAND ----------

df_ff_trab_consolidado.drop("MES_CADASTRO")
...
df_ff_trab_consolidado = spark.sql("""
    	SELECT A.*
			...
			--,B.NOVO_TERCEIRO_1
            ,B.TERCEIRO_NOVO
			,(CASE WHEN B.TERCEIRO_PRINCIPAL IS NULL THEN B.TERCEIRO_NOVO
...
df_ff_trab_consolidado = df_ff_trab_consolidado.withColumnRenamed("PARTE_CONTRARIA_CARGO_GRUPO", "PARTE_CONTRARIA_CARGO_GRUPO_1")
...
df_ff_trab_consolidado = df_ff_trab_consolidado.withColumn("PARTE_CONTRARIA_CARGO_GRUPO_1", 
                   expr("""
                        CASE WHEN PARTE_CONTRARIA_CARGO_GRUPO_1 LIKE '% PARA'
                        THEN SUBSTRING(PARTE_CONTRARIA_CARGO_GRUPO_1, 1, LENGTH(PARTE_CONTRARIA_CARGO_GRUPO_1) - 5)
                        ELSE PARTE_CONTRARIA_CARGO_GRUPO_1 END
                        """
                    )
                   )
...
# Cria a view temporária do dataframe final
df_ff_trab_cons_pr66.createOrReplaceTempView("VERIFICACAO_FINAL_1")
...
# MAGIC FROM VERIFICACAO_FINAL_1
...
colunas_existentes = df_ff_trab_cons_pr66.columns
...
df_ff_trab_cons_pr66 = convert_to_date_format(df_ff_trab_cons_pr66, df_ff_trab_cols)
...
# pandas_df = df_ff_trab_cons_pr66.toPandas()

# # Salvar o DataFrame com os nomes corrigidos em um arquivo Excel no disco local primeiro
# local_path = f'/local_disk0/tmp/df_ff_trab_cons_pr66.xlsx'
# pandas_df.to_excel(local_path, index=False, sheet_name='Teste')

# # Copiar o arquivo do disco local para o volume desejado
# volume_path = f'/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_fechamento_financeiro/trabalhista_base_financeiro/df_ff_trab_cons_pr66.xlsx'
...
# # Converter PySpark DataFrame para Pandas DataFrame
# pandas_df = df_ff_trab_cons_pr88.toPandas()

# # Salvar o DataFrame com os nomes corrigidos em um arquivo Excel no disco local primeiro
# local_path = f'/local_disk0/tmp/df_ff_trab_cons_pr88.xlsx'
# pandas_df.to_excel(local_path, index=False, sheet_name='Teste')

# # Copiar o arquivo do disco local para o volume desejado
# volume_path = f'/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_fechamento_financeiro/trabalhista_base_financeiro/df_ff_trab_cons_pr88.xlsx'

# copyfile(local_path, volume_path)

# COMMAND ----------

df_ff_trab_cons_pr99 = merge_dfs(df_ff_trab_cons_pr, df_ff_trab_cons_pr88, ['ID_PROCESSO', 'MES_FECH'])
...
# df_ff_trab_cons_pr99.createOrReplaceTempView("TB_FECHAM_TRAB_99")

# df_ff_trab_cons_pr999 = spark.sql(f"""
# SELECT *
# FROM
#   TB_FECHAM_TRAB_99
...
# # Converter PySpark DataFrame para Pandas DataFrame
# pandas_df = df_ff_trab_cons_pr999.toPandas()

# # Salvar o DataFrame com os nomes corrigidos em um arquivo Excel no disco local primeiro
# local_path = f'/local_disk0/tmp/df_ff_trab_cons_pr999.xlsx'
# pandas_df.to_excel(local_path, index=False, sheet_name='Teste')

# # Copiar o arquivo do disco local para o volume desejado
# volume_path = f'/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_fechamento_financeiro/trabalhista_base_financeiro/df_ff_trab_cons_pr999.xlsx'
...
display(df_ff_trab_cons_pr8.select("ID_PROCESSO", "MES_FECH").limit(10))

# COMMAND ----------

df_ff_trab_cons_pr8.createOrReplaceTempView("DF_FECHAMENTO_TRAB")
...
SELECT ID_PROCESSO
        ...
        ,ANO_AGING_ESTOQ
        ,AGING_ESTOQ_MESES
        ,BASE_ANO_AGING_ESTOQ
        ,COUNT(ID_PROCESSO) AS QTD
        FROM DF_FECHAMENTO_TRAB
...
df_consulta = spark.sql("""
/* CARREGA AS INFORMAÇÕES ADICIONAIS NA BASE DO FECHAMENTO */
SELECT INDICACAO_PROCESSO_ESTRATEGICO
        ,COUNT(ID_PROCESSO) AS QTD
FROM TB_FECH_FIN_TRAB_CONSOLIDADO_99
GROUP BY 1;
""")
...
# ,AGING_ESTOQ_MESES
# FROM
# TB_FECH_FIN_TRAB_CONSOLIDADO_8
# WHERE MES_CADASTRO IS NOT NULL
# LIMIT 100
...
# df_test.where("ANO_AGING_ENCERR IS NOT NULL").limit(200).display()
...
# MAGIC %sql
# MAGIC -- SELECT DISTINCT FASE, DP_FASE FROM TB_FECH_FIN_TRAB_CONSOLIDADO_9 LIMIT 100
...
# MAGIC %sql
# MAGIC -- SELECT DISTINCT VP, DIRETORIA, DP_VP_DIRETORIA FROM TB_FECH_FIN_TRAB_CONSOLIDADO_10
...
df_ff_trab_cons_pr11 = spark.sql(
  """
  SELECT *,
    CASE 
...
df_ff_trab_cons_pr11.createOrReplaceTempView("TB_FECH_FIN_TRAB_CONSOLIDADO_11")
...
#   DP_NATUREZA
# FROM
#   TB_FECH_FIN_TRAB_CONSOLIDADO_11 
# WHERE  
#   PROVISAO_TOTAL_PASSIVO_M > 0
...
FROM
  TB_FECH_FIN_TRAB_CONSOLIDADO_12
"""
)
df_ff_trab_cons_pr17 = df_ff_trab_cons_pr17.dropDuplicates(["MES_FECH", "ID_PROCESSO", "NOVOS", "ENCERRADOS", "ESTOQUE"])
df_ff_trab_cons_pr17.createOrReplaceTempView("TB_FECH_FIN_TRAB_CONSOLIDADO_17")

# COMMAND ----------

# df_ff_trab_cons_pr17.count()
...
#   MOTIVO_ENC_AGRP
# FROM
#   TB_FECH_FIN_TRAB_CONSOLIDADO_17
# WHERE
#   MOTIVO_ENC_AGRP IS NOT NULL
# LIMIT
#   100
...
SELECT
  *,
  CASE
    WHEN PARTE_CONTRARIA_CARGO_GRUPO_1 IN (
      'AJUDANTE',
      'AJUDANTE EXTERNO',
      'ANALISTA',
      'AUXILIAR',
...
# MAGIC %sql
# MAGIC -- SELECT DISTINCT PARTE_CONTRARIA_CARGO_GRUPO, CARGO_TRATADO FROM TB_FECH_FIN_TRAB_CONSOLIDADO_18
...
FROM TB_FECH_FIN_TRAB_CONSOLIDADO_18
"""
)
df_ff_trab_cons_pr19.createOrReplaceTempView("TB_FECH_FIN_TRAB_CONSOLIDADO_19")
...
# MAGIC %sql
# MAGIC -- SELECT DISTINCT AGING_ESTOQ_MESES, MESES_AGING_ENCERR, CLUSTER_AGING FROM TB_FECH_FIN_TRAB_CONSOLIDADO_19
...
df_ff_trab_cons_pr20 = spark.sql("""
SELECT
  A.*,
  b.PARA AS CLUSTER_VALOR
FROM
  TB_FECH_FIN_TRAB_CONSOLIDADO_19 A
  LEFT JOIN TB_DP_CLUSTER_VALOR B ON A.ESTADO = B.DE
  """
)
df_ff_trab_cons_pr20.createOrReplaceTempView("TB_FECH_FIN_TRAB_CONSOLIDADO_20")

# COMMAND ----------

# MAGIC %sql
# MAGIC -- SELECT DISTINCT ESTADO, CLUSTER_VALOR FROM TB_FECH_FIN_TRAB_CONSOLIDADO_20
...
df_ff_trab_cons_pr21 = spark.sql("""
SELECT
A.*,
    CASE
...
FROM TB_FECH_FIN_TRAB_CONSOLIDADO_20 A
"""
)
df_ff_trab_cons_pr21.createOrReplaceTempView("TB_FECH_FIN_TRAB_CONSOLIDADO_21")
...
# MAGIC --   DATA_ADMISSAO,
# MAGIC --   DATA_DISPENSA,
# MAGIC --   TEMPO_EMPRESA_MESES,
# MAGIC --   SAFRA_RECLAMACAO
# MAGIC -- FROM TB_FECH_FIN_TRAB_CONSOLIDADO_21
...
df_ff_trab_cons_pr21 = spark.sql("""
	...
	FROM TB_FECH_FIN_TRAB_CONSOLIDADO_21 A
	LEFT JOIN TB_DP_TERCEIROS B ON A.TERCEIRO_AJUSTADO=B.`EMPRESA TERCEIRIZADA`
 """
)

df_ff_trab_cons_pr21.createOrReplaceTempView("TB_FECH_FIN_TRAB_F")

# COMMAND ----------

# Cria a view temporária do dataframe final
df_ff_trab_cons_pr21.createOrReplaceTempView("VERIFICACAO_FINAL")
...
df_ff_trab_cons_pr21.write \
  .format("delta") \
  .mode("overwrite") \
  .option("overwriteSchema", "true") \
  .saveAsTable(f"databox.juridico_comum.tb_fecham_financ_trab_{nmmes}")

# COMMAND ----------

#nmmes = dbutils.widgets.get("nmmes")

#df_ff_trab_cons_pr21.write.format("delta").mode("overwrite").saveAsTable(f"databox.juridico_comum.tb_fecham_financ_trab_{nmmes}")
...
tb_inss_encerr_trab_202511_f = spark.sql(f"""
SELECT
   ID_PROCESSO
	,MES_CADASTRO
	,VALOR_DA_CAUSA
...
FROM
  ...
  WHERE ENCERRADOS = 1 AND MES_FECH >= '2025-11-24'
"""
)
...
# Converter PySpark DataFrame para Pandas DataFrame
pandas_df = tb_inss_encerr_trab_202510_f.toPandas()

# Salvar o DataFrame com os nomes corrigidos em um arquivo Excel no disco local primeiro
local_path = f'/local_disk0/tmp/tb_inss_encerr_trab_202510_f.xlsx'
pandas_df.to_excel(local_path, index=False, sheet_name='TB_FECHAM_FINANC_TRAB')

# Copiar o arquivo do disco local para o volume desejado
volume_path = f'/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_indicadores_resultado/tb_inss_encerr_trab_202511_f.xlsx'

copyfile(local_path, volume_path)

# COMMAND ----------

# display(tb_inss_encerr_trab_202406_f)

# COMMAND ----------

# teste = spark.sql(f"SELECT DATA_ADMISSAO FROM databox.juridico_comum.tb_fecham_financ_trab_{nmmes}")
