...
comps = ['202110', '202111', '202112','202201', '202202', '202203', '202204', '202205', '202206', '202207', '202208', '202209', '202210', '202211', '202212', '202301', '202302', '202303', '202304', '202305', '202306', '202307', '202308', '202309', '202310', '202311', '202312', '202401', '202402', '202403', '202404','202405','202406','202407','202408','202409','202410']

df_fech_trab_provisao = spark.read.table(f"databox.juridico_comum.tb_fecham_trab_202410").limit(0)
...
# MAGIC %sql
# MAGIC
# MAGIC select * from df_pgto_garantias_somado_f

# COMMAND ----------

df_fin_trab_provisao_pgto = spark.sql('''
    SELECT * FROM df_pgto_garantias_somado_f A 
    LEFT JOIN TB_FECH_FIN_TRAB_PROVISAO_66 B
    ON A.PROCESSO_ID = B.ID_PROCESSO AND A.MES_PGTO = B.MES_FECH
''')

df_fin_trab_provisao_pgto.createOrReplaceTempView('df_fin_trab_provisao_pgto')


# COMMAND ----------

df_fin_trab_provisao_pgto_f = spark.sql('''
    select * from df_fin_trab_provisao_pgto
    where id_processo is not null
    and LINHA NOT LIKE '%linha01%'
''')

df_fin_trab_provisao_pgto_f.createTempView('df_fin_trab_provisao_pgto_f')
...
#df_fin_trab_provisao_pgto_f

# COMMAND ----------

# MAGIC %md
# MAGIC #### Exportação de arquivos
...
# Converter PySpark DataFrame para Pandas DataFrame
pandas_df = df_fin_trab_provisao_pgto_f.toPandas()

# Salvar o DataFrame com os nomes corrigidos em um arquivo Excel no disco local primeiro
local_path = f'/local_disk0/tmp/df_fin_trab_provisao_pgto_f_{mes_fechamento}_2.xlsx'
pandas_df.to_excel(local_path, index=False, sheet_name='TB_FECHAM_FINANC_TRAB')

# Copiar o arquivo do disco local para o volume desejado
volume_path = f'/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_fechamento_financeiro/trabalhista_base_financeiro/df_fin_trab_provisao_pgto_f_{mes_fechamento}_2.xlsx'
