...
# MAGIC # Base Ativos
# MAGIC
# MAGIC Agregação das tabelas do RH com infos adicionais.
...
rh_path = '/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_rh/fechamento/'
...
df_ativos = spark.sql("""
	SELECT CODCCUSTOCONTABIL --AS CodCCustoContabil
			,MES_REF
			,CODEMPRESA
			,DESCRICAON1
			,DESCRICAON2
	...
	FROM ATIVOS_RH_HIST
	WHERE UPPER(STATUS) IN ('ATIVO','AFASTADO')
	GROUP BY ALL;
 """)

df_ativos.createOrReplaceTempView("ATIVOS_RH_HIST")
...
df_ativos1.createOrReplaceTempView("ATIVOS_RH_HIST1")
...
local_path = f'/local_disk0/tmp/TB_ATIVOS_FINAL.xlsx'
...
volume_path = f'/Volumes/databox/juridico_comum/arquivos/trabalhista/bases_indicadores_resultado/ATIVOS_FINAL.xlsx'
